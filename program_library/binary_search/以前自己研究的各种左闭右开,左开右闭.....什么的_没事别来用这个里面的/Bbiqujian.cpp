...
int main(){
	...
	static Road*road=(Road*)malloc(1000100*sizeof(Road));
	for(int i=0;i<m;i++){
...
,int be_searched){
	...
	for(;1<high-low;){
		...
		(be_searched<data[middle].from)?high=middle-1:low=middle;
	}
	if(be_searched==data[low].from){
		for(;be_searched==data[low].from;low=low-1);
		low=low+1;
		return low;
	}else{
		return 10000010;
